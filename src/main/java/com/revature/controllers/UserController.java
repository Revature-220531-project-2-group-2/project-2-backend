 ...
 * 
 * 
 * User can: register * be updated * be deleted * get a list of all users get a
 * user by id "/id-{id} get a user by username "/{username} create a new
 * character * "/{username}/add-character"
 * 
 * update/save a specific character * "/{username}/update-character" remove a
 * specific character * "/{username}/remove-character" retrieve their characters
 * * "/{username}/characters" retrieve a character by their name *
 * "/{/{username}/characters/{charname}"
 * 
 * 
 * retrieve a list of their campaigns * "/{username}/campaigns retrieve a
 * specific campaign user is in "/{username}/campaign-{id} add themselves to a
 * specific campaign* "/{username}/campaigns/join-campaign-{id} can view all
 * campaigns * "/{username}/view-all-campaigns"
 * 
 * 
 * 
...
//===========================================GENERAL USER MAPPINGS ============================================
	 ...
	 */
	@PostMapping
	public User registerNewUser(@RequestBody UserHolder newUserInfo) {
		User u = new User();
		return userService.processRegister(User.makeUserFromUserHolder(u, newUserInfo));
	}
...
//=================================User Campaign Mappings =====================================================
	...
	public ResponseEntity<User> addNewCharacter(@PathVariable("username") String username,
			@RequestBody CharSheetHolder charSheetInfo) {
		...
		if (!user.isPresent()) {
			...
			userService.addCharSheet(user.get(), CharSheet.copyCharSheetInfoFromHolder(charSheet, charSheetInfo));
			return ResponseEntity.ok(user.get());
		}
	...
	@PostMapping(value = "/{username}/delete-character-{id}")
	public ResponseEntity<User> removeCharSheet(@PathVariable("username") String username,
			@PathVariable("id") int charId) {

		Optional<User> user = userService.getByUsername(username);
		Optional<CharSheet> character = charService.getCharacterById(charId);

		if (!user.isPresent() || !character.isPresent()) {
			return new ResponseEntity<User>(HttpStatus.NO_CONTENT);
		} else {
			User u = user.get();
			userService.removeCharSheet(u, character.get());
			return ResponseEntity.ok(user.get());
		}

	}

	@PostMapping(value = "/{username}/delete-{charName}")
	public ResponseEntity<User> removeCharSheet(@PathVariable("username") String username,
	...
	/**
	 * Update the users character by id
	 * 
	 * @param username
	 * @param updatedCharSheet Character Sheet containing all of the changes
	 */
	@PutMapping(value = "/{username}/update-character-{id}")
	public void updateCharSheet(@PathVariable("username") String username, @PathVariable("id") int charId,
			@RequestBody CharSheetHolder updatedCharInfo) {
		...
		Optional<CharSheet> charSheet = charService.getCharacterById(charId);
		if(!charSheet.isPresent()) return;
		
		CharSheet updatedCharSheet = CharSheet.copyCharSheetInfoFromHolder(charSheet.get(), updatedCharInfo);
